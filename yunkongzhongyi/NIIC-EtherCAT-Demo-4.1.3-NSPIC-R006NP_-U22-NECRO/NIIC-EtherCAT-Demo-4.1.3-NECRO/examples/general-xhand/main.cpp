...
printf("get pdo rrrrrrrrrrrrrrrrrrrrxEntries--------------------------------------------------------------------------\n");
        std::vector<ecat::pdo_entry_info> rxEntries = task.get_io_rx_config(slave_pos, 0);
        for (auto& pdo : rxEntries) {
          if (pdo.entry_idx.idx == 0)
            continue;
        printf("pdo.entry_idx.sub_idx %d\n",pdo.entry_idx.sub_idx);
        printf("pdo.datatype %s\n",pdo.datatype.c_str());
        printf("pdo.bit_len %d\n",pdo.bit_len);
        printf("pdo.name %s\n",pdo.name.c_str());
...
printf("get pdo tttttttttttttttttttxEntries--------------------------------------------------------------------------\n");
