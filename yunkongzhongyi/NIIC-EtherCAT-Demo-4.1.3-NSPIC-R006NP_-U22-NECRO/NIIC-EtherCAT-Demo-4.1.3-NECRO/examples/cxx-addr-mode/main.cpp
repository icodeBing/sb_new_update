...
void init_slave(int slave_pos, int op_mode)
{
  auto profile_no = tt->profile_no(slave_pos);
  //CiA402协议模式
  if (profile_no == 402)
  {
    int n_axis_in_slave = 1, slot_pos;
    int slots_count = tt->slots_count(slave_pos);
    int slots_index_increment = tt->slot_index_increment(slave_pos);
    if (slots_count > 0)
    {
  ...
  if (profile_no == 5001)
  {
  ...
  #if 1
    std::vector<ecat::pdo_entry_idx> entries = tt->mapped_pdo_entries(slave_pos);
    ...
    for (int j = 0; j < entries.size(); j++) {
      ...
      if (entries[j].idx >= 0x6000 && entries[j].idx < 0x7000)
      {
        ...
        tt->try_register_pdo_entry(io->io_address, slave_pos, entries[j], &io->io_bit_pos);
    ...
    if (slave_pos == 1)
    {
      ...
      io->slave_pos = slave_pos;
      io->io_id = 1;
      tt->try_register_pdo_entry(io->io_address, slave_pos, { 0x7000, 0x01 }, & io->io_bit_pos);
      programs.emplace_back(program_output{ io.get() });
      io_output.push_back(std::move(io));
      ...
      io1->io_id = 2;
      tt->try_register_pdo_entry(io1->io_address, slave_pos, { 0x7001, 0x01 }, &io1->io_bit_pos);
      ...
      io2->io_id = 3;
      tt->try_register_pdo_entry(io2->io_address, slave_pos, { 0x7002, 0x01 }, &io2->io_bit_pos);
      ...
      // io3->io_id = 4;
      // tt->try_register_pdo_entry(io3->io_address, slave_pos, { 0x7003, 0x01 }, &io3->io_bit_pos);
    ...
    if (slave_pos == 2)
    {
      ...
      io->slave_pos = slave_pos;
      io->io_id = 1;
      tt->try_register_pdo_entry(io->io_address, slave_pos, { 0x6000, 0x01 }, & io->io_bit_pos);
      programs.emplace_back(program_input{ io.get() });
      io_input.push_back(std::move(io));
      ...
      io1->slave_pos = slave_pos;
      io1->io_id = 2;
      tt->try_register_pdo_entry(io1->io_address, slave_pos, { 0x6001, 0x01 }, & io1->io_bit_pos);
      ...
      io2->slave_pos = slave_pos;
      io2->io_id = 3;
      tt->try_register_pdo_entry(io2->io_address, slave_pos, { 0x6002, 0x01 }, & io2->io_bit_pos);
      ...
      io3->slave_pos = slave_pos;
      io3->io_id = 4;
      tt->try_register_pdo_entry(io3->io_address, slave_pos, { 0x6003, 0x01 }, & io3->io_bit_pos);
...
int main(int argc, const char **argv)
{
  ...
  opts.add_options()
      ...
      ("addr_mode,d", boost::program_options::value<int>(&addr_mode)->default_value(0), "Set master addressing mode");
  ...
  task.set_config_callback([&] {
    ...
    std::vector<ecat::slave_by_alias> alias_count= task.slave_alias_count();
    // task.get_master_ptr()->set_addr_mode(static_cast<ecat::addr_mode>(addr_mode));
    // task.get_master_ptr()->write_slave_alias(0, 1);
    // addr_mode=task.get_master_ptr()->get_addr_mode();
    if(addr_mode==ecat::addr_mode::logical_addressing)
    {
      for(slave_pos = 0; slave_pos < slave_count; slave_pos++)
      {
        init_slave(slave_pos, op_mode);
      }
    }
    else if(addr_mode==ecat::addr_mode::alias_addressing)
    {
      for(auto iter : alias_count)
      {
        slave_pos=task.get_slave_by_alias(iter);
        init_slave(slave_pos, op_mode);
      }
    }
    else if(addr_mode==ecat::addr_mode::logical_alias_addressing)
    {
      std::vector<ecat::slave_by_alias> alias_count= task.slave_alias_count();
      auto alias_slave=task.get_slave_by_alias(TARGET_ALIAS);
      init_slave(alias_slave, op_mode);
      for(slave_pos = 0; slave_pos < slave_count; slave_pos++)
      {
        if(slave_pos==alias_slave)
          continue;
        init_slave(slave_pos, op_mode);
      }
    }
