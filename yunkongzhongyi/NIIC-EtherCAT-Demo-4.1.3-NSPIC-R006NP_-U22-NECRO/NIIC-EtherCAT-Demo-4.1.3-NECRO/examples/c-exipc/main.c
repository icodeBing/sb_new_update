...
static void config_cb(void *user_data)
{
  ...
  for (i = 0; i < slave_count; ++i)
  {
    ...
    if (profile_no == 402)
    {
      ...
      for (slot_pos = 0; slot_pos < n_axis_in_slave; ++slot_pos)
      {
        ...
        ec_task_axis_name(task, i, pGroupInfo->info[axis_count].name);
        pGroupInfo->info[axis_count].VID = ec_task_axis_vid(task, i);
        pGroupInfo->info[axis_count].PID = ec_task_axis_pid(task, i);

        // Set mode_of_opreation SDOpIpcEvtGroupInfo->groupInfo
  ...
  pIpcEvtGroupSta->ID = (sizeof(ServoSta_t)*pGroupSta->slotCount+4) | SERVO_GOURP_STA;
  pIpcEvtGroupInfo->ID = (sizeof(ServoInfo_t)*pGroupInfo->slotCount+4) | SERVO_GROUP_INFO;
...
static void axis_qt_control(struct axis_data *axis, int i)
{
  if (servoSta[i].powerON != 0)
  {
    if (axis_enable(axis))
    {
      axis->target_velocity_buffer = 1310720;
      if (axis->position_actual_value_buffer != servoSta[i].targetPosition){
        int d = servoSta[i].targetPosition - axis->position_actual_value_buffer;
        if (abs(d) > 10000){
          axis->p_buf = d > 0 ? 1000 : -1000;
          axis->target_position_buffer += axis->p_buf;
        }
        else {
          axis->target_position_buffer = servoSta[i].targetPosition;
        }
  ...
  pGroupSta->sta[i].axisCount = i;
  pGroupSta->sta[i].errorCode = axis->error_code_buffer;
  pGroupSta->sta[i].statusWord = axis->status_word_buffer;
  pGroupSta->sta[i].curMode = axis->mode_of_operation_display_buffer;
  pGroupSta->sta[i].curPos = axis->position_actual_value_buffer;
  pGroupSta->sta[i].curVel = axis->velocity_actual_value_buffer;
}

static void axis_qt_demo(struct axis_data *axis, int i)
{
  axis_control_demo(axis);
...
static void send_cb(void *user_data)
{
  ...
  for (i = 0; i < axis_count; ++i)
  {
    if (demo_flag == 1)
    {
      axis_qt_demo(&axes[i], i);
    }
    else
    {
      if (al_order != NULL)
      {
        if (al_order->axisCount == i)
        {
          servoSta[i].axisCount = al_order->axisCount;
          servoSta[i].powerON = al_order->powerON;
          servoSta[i].targetPosition = al_order->targetPosition;
          servoSta[i].targetVelocity = al_order->targetVelocity;
          ec_algo_ipc_del(al_order);
        }
      }
      axis_qt_control(&axes[i], i);
    }
  }
...
//     if (fdd2 != NULL)
//       fwrite("order\n", 1, 6, fdd2);
//     if (ec_ipc_front()->targetVelocity == 10086)
//     {
//       demo_flag = ec_ipc_front()->powerON;
//       ec_ipc_pop();
//       return;
//     }

//     int step = 2;
//     int curPos = pGroupSta->sta[ec_ipc_front()->axisCount].curPos;
//     int pos = (ec_ipc_front()->targetPosition - curPos) / step;
...
//       if (i == step-1)
//         tmp.targetPosition = ec_ipc_front()->targetPosition;
//       tmp.axisCount = ec_ipc_front()->axisCount;
//       tmp.targetVelocity = ec_ipc_front()->targetVelocity;
//       tmp.powerON = ec_ipc_front()->powerON;
...
int main(int argc, char *argv[])
{
  ...
  while ((c = getopt_long(argc, argv, "f:c:hl:p:a:d:", long_options,
                          &option_index)) != -1)
  {
    switch (c)
    {
