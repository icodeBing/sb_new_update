    ...
    void operator()() {
        ...
        if (!(count % 1000))
        {
           uint16_t val = *io->io_address;
            std::cout << "read slave: " << io->slave_pos << "io: " << io->io_idx << " value is " << val << "\n";
        }
        count %= 10000;
    }
};
...
public:
    ...
    bool startAsSlave(MasterProgram* master, int num);
    void wait();
    void release();
