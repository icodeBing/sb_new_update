...
Motor_Ctrl_Mode_type Ti5_Motor_Ctrl_Mode = Motor_Ctrl_Mode_type::Force_Pos_Mode; // 默认力位混控模式，如果需要调整电机控制模式，修改改变量即可
// Motor_Ctrl_Mode_type Ti5_Motor_Ctrl_Mode = Motor_Ctrl_Mode_type::Pos_Mode; // 
...
void power::on_cycle()
{
  ...
  // __RT(printf("target_position: ",*axis->target_position,"actual_position: ",*axis->position_actual_value));
  // std::cout<<"id: "<<axis->axis_id<<"   target_position: "<<*axis->target_position<<"       actual_position: "<<*axis->position_actual_value<<std::endl;
  if (!status)
  {
    ...
    {// 力位混控模式
        *axis->target_pos = *axis->actual_pos;
    }
    ...
    *axis->control_word << *command;
    ...
    if (state == cia402_state::switched_on)
    {
      if (axis->mode_of_operation)
      {
        if (Ti5_Motor_Ctrl_Mode == Motor_Ctrl_Mode_type::Pos_Mode)
        {// 位置控制模式
            *axis->mode_of_operation = op_mode_csp;
        }
        else if (Ti5_Motor_Ctrl_Mode == Motor_Ctrl_Mode_type::Force_Pos_Mode)
        {// 力位混控模式
            *axis->mode_of_operation = op_mode_pt;
        }
      }
    }
  }
  ...
  if (old_state != state)
  {
    spdlog::debug("Axis {} changed from '{}' to '{}', control_word=0x{:X}",
      axis->axis_id,
      to_string(old_state),
      to_string(state),
      *axis->control_word);
