...
void IoController::on_cycle_encos(int slave_pos)
{
    ...
    {                                                          // ID设置好后，进行电机控制程序
        ...
        { // slave_pos=0表示主站0连接的ethercat转can模块的第一块板子，带6个电机
            ...
            // for(int i=0; i<6 ;i++)
            // {
            //     std::cout<<"encos_motor_num_slave0:" << i <<"...."<< motor_msg.master_id[0].motor_id[i].position_cmd<<std::endl;
            // }
...
void IoController::MotorZeroSetting(EtherCAT_Msg *TxMessage, uint16_t motor_id, int passage)
{
    ...
    // ENCOS 5.2 电机零点设置指令:
    // Byte0~1: motor_id, Byte2: 0x00, Byte3: 0x03
    // 发送前请确保电机停稳，并与上一条设置类命令至少间隔 500ms。
    TxMessage->motor[0].data[0] = motor_id >> 8;
    TxMessage->motor[0].data[1] = motor_id & 0xff;
    TxMessage->motor[0].data[2] = 0x00;
    TxMessage->motor[0].data[3] = 0x03;
