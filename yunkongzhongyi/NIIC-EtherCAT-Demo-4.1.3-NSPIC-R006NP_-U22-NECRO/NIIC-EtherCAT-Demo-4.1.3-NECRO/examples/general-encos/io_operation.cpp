...
void IoController::send_motor_ctrl_cmd(EtherCAT_Msg *TxMessage,uint8_t passage,uint16_t motor_id, float kp, float kd, float pos, float spd, float tor, int slave_pos)
{
    ...
    if(motor_id == 5 && count_flag%1000==0)
    {
        __RT(printf("[motor5]data0=%d\n", TxMessage->motor[passage-1].data[0]));
        __RT(printf("[motor5]data1=%d\n", TxMessage->motor[passage-1].data[1]));
        __RT(printf("[motor5]data2=%d\n", TxMessage->motor[passage-1].data[2]));
        __RT(printf("[motor5]data3=%d\n", TxMessage->motor[passage-1].data[3]));
        __RT(printf("[motor5]data4=%d\n", TxMessage->motor[passage-1].data[4]));
        __RT(printf("[motor5]data5=%d\n", TxMessage->motor[passage-1].data[5]));
        __RT(printf("[motor5]data6=%d\n", TxMessage->motor[passage-1].data[6]));
        __RT(printf("[motor5]data7=%d\n", TxMessage->motor[passage-1].data[7]));
    }
...
void IoController::on_cycle_encos(int slave_pos) {
    ...
    // send_motor_ctrl_cmd(&Tx_Message, 5,5, 0, 0, 0, 0, 50, slave_pos);
    // send_motor_ctrl_cmd(&Tx_Message, 6,6, 0, 0, 0, 0, 100, slave_pos);
    send_motor_ctrl_cmd(&Tx_Message, 4,4, 0, 0, 0, 0, 5, slave_pos);
    send_motor_ctrl_cmd(&Tx_Message, 1,6, 0, 0, 0, 0, 5, slave_pos);
