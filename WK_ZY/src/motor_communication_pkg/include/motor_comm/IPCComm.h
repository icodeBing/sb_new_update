...
private:
    ...
    int32_t float_to_int(float data);
    float int_to_float(int32_t data);

    int32_t ti5_rad_to_pulse(double rad);
    int32_t ti5_angular_velocity_to_pulse(double angular_velocity);
    float Ti5_current_to_effort(uint8_t master_id, uint8_t motor_idx, int32_t current_actual);
private:
    ...
    Motor_Ctrl_Mode_type m_Motor_Ctrl_Mode = Motor_Ctrl_Mode_type::Force_Pos_Mode; // 英克斯电机默认力位混控模式，如果需要调整电机控制模式，修改改变量即可
    // Motor_Ctrl_Mode_type Ti5_Motor_Ctrl_Mode = Motor_Ctrl_Mode_type::Pos_Mode; // 
    Motor_Ctrl_Mode_type Ti5_Motor_Ctrl_Mode = Motor_Ctrl_Mode_type::Force_Pos_Mode; // 钛虎电机默认力位混控模式，如果需要调整电机控制模式，修改改变量即可
